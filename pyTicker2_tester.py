...
def make_NRPR_body(EX, RP, C, TU, nu):
    rows = []
    for tu in TU:
        for rp in RP:
...
if __name__ == "__main__": 
    if(args.input == None):
        ...
        EX, C, nu, TU, SU, RP = parse_config(cfg)

        while(True):
            try:
                #async_run(EX, C)
                run(EX,C)
                print_table(EX=EX,C=C,SU=SU,TU=TU,nu=nu,RP=RP)
