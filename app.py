...
currencies = ["ETH", "DASH", "LTC", "ETC", "ZEC", "XRP", "GNT",  "GNO"]
cols = ["ETH","DASH","LTC","ETC","ZEC", "XRP","GNT",  "GNO", "BTC"]
duration = 1 #sec
...
def run_ticker(sc): 
    try:
        ...
        if args.co:
            ...
            if(args.alarm):
                if(len({k:v for (k,v) in CP_diff_last.items() if v <= 99 and polo_percent_changes[k] > 0}) > 0):
                    pygame.mixer.music.play()
